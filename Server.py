...
def on_connect(client, userdata, flags, rc):
    ...
    return
def ret_msg(client, userdata, msg):
    name = msg.payload.decode("utf-8")
    tname = 'login@'+name
    tps = dbser.getUT(name)
    for tp in tps:
        tdata = dbser.getMsg(tp)
        for d in tdata:
            mssg = tp+'$'+d['sender']+'$'+d['msg']
            client.publish(tname,mssg)
    return
...
def auth(client, userdata, msg):
    ...
    if(tmp[1] in ser):
        if(tmp[2]==serpass[tmp[1]]):
            if(tmp[0] not in ser[tmp[1]]):
                ser[tmp[1]].append(tmp[0])
                dbser.addTU(tmp[1],tmp[0])
        ...
        if(key==msg.topic and  tmp[0] in ser[key]):#[1:]
            if(len(tmp)>2):
                for i in range(2,len(tmp),1):
                    tmp[1]+="<@>"
                    tmp[1]+=tmp[i]
            dbser.addMsg(msg.topic,tmp[0],tmp[1])
...
temp = dbser.getTUu('topic')
for t in temp:
    ser[t] = dbser.getTU(t)
    serpass[t] = dbser.getPass(t)
...
client.message_callback_add("login_succ",ret_msg)
